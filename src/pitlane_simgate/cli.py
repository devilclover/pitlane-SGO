...
from .scenario import scenario_from_log, save_scenario, load_scenario
from .utils import mkdirp, parse_grid, product_grid, json_dump, sha256_file
from .models import RunSpec, RunResult, GateDecision, Metrics
from .runner import run_sweep
from .gate import load_gate_rules, evaluate_runs
from .report import write_report
from .crypto import ensure_keys, sign_payload
from .ros2_adapter import scenario_from_ros2_bag, emit_ignition_world
...
def from_log(
    ...
    default_params: List[str] = typer.Option([], help="Default params like key=val (floats or strings)"),
):
    params: Dict[str, Any] = {}
...
def ros2_scenario(
    ...
    mkdirp(os.path.dirname(scenario_out) or ".")
    save_scenario(sc, scenario_out)
    rprint({"scenario_out": scenario_out, "source_hash": sc.source_hash, "topics": sc.metadata.get("topics", [])[:3]})
...
def sweep(
    ...
    grid: str = typer.Option("", "--grid", help="Param grid like 'speed=0.6..1.2:4; friction=0.8,1.0'"),
    driver: str = typer.Option("dummy", help="dummy | shell"),
    shell_cmd: Optional[str] = typer.Option(None, help="Command for shell driver; must write $SIM_OUT JSON"),
    ...
    results: List[RunResult] = run_sweep(sc, runs, driver=driver, shell_cmd=shell_cmd, work_dir=work_dir)
    json_dump([{"run_id": r.run_id, "scenario_id": r.scenario_id, "params": r.params, "metrics": r.metrics.__dict__} for r in results], out_json)
    rprint({"runs": len(results), "out_json": out_json, "hint": "next: simgate evaluate --results ... --gates ... --out-html report.html"})
...
def evaluate(
    ...
    raw = json.load(open(results, "r", encoding="utf-8"))
    runs = []
    for it in raw:
    ...
    payload = {
        ...
        "decision": decision.__dict__ | {"gate_results": [gr.__dict__ for gr in decision.gate_results]},
    ...
    json_dump(decision.__dict__ | {"gate_results": [gr.__dict__ for gr in decision.gate_results]}, out_decision)
    json_dump(att, out_attestation)
    rprint({"overall_pass": decision.overall_pass, "action": decision.action, "report": out_html, "attestation": out_attestation})

@app.command()
def verify(attestation: str = typer.Option(..., help="decision.attestation.json")):
    data = json.load(open(attestation, "r", encoding="utf-8"))
